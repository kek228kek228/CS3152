...
public class GameCanvas {
	...
	public void draw(TextureRegion region, Color tint, float ox, float oy, 
        ...
        for(int i=-1; i<2;i++){
            for(int j=-1;j<2;j++){
                computeTransform(ox, oy, x+this.width*i, y+this.height*j, angle, sx, sy);
...
//		spriteBatch.draw(region, region.getRegionWidth(), region.getRegionHeight(), local);
	...
	public void drawOverlay(Texture image, Color tint, boolean fill, int time) {
		if (!active) {
			Gdx.app.error("GameCanvas", "Cannot draw without active begin()", new IllegalStateException());
			return;
		}
...
//		computeTransform(0, 0, 0, 0, time%360, 2.0f, 0.1f);
		spriteBatch.setColor(tint);
		spriteBatch.draw(image, (float)0,(float)((height/2)-(width/2)),width/2,width/2,(float) (width),(float) (width),1.3f,1.3f,(float)(time*0.2)%360,0,0,900, 600,false,false);
	}
