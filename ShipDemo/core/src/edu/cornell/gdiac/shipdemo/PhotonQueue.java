...
public class PhotonQueue {
	...
	public class Photon {
		 ...
		 */
		public void allocate(float x, float y, float vx, float vy, boolean type) {
			this.x  = x;  this.y  = y;
	 ...
	 */
	public void addPhoton(Vector2 position, Vector2 velocity, float angle, boolean type, boolean homing, Vector2 position1) {
		...
		if (homing){
			fire_x=velocity.x + (float)Math.cos(Math.toRadians((int)(Math.atan2((position1.y-position.y),(position1.x-position.x))/Math.PI*180)))  * PHOTON_VELOCITY;
			fire_y=velocity.y + (float)-Math.sin(Math.toRadians((int)(Math.atan2((position.y-position1.y),(position.x-position1.x))/Math.PI*180))) * PHOTON_VELOCITY;
		}
//		int a=(int)(Math.atan2((position.x-position1.x),(position.y-position1.y))/Math.PI*180);
	...
	public void draw(GameCanvas canvas) {
        ...
        for (int ii = 0; ii < size; ii++) {
			...
			if(!queue[idx].type) {
				...
				float f = 1.0f - (float) queue[idx].age * 0.015f / ((float) MAX_AGE * 0.02f);
				Color c = new Color(f,f,f,f);
				// Use this information to draw.
				canvas.draw(texture, c, ox, oy, queue[idx].x, queue[idx].y, 0, scale, scale);
			}
			else{
				float scale = 2.5f - (float) queue[idx].age * 1.0f / (float) MAX_AGE;
				float f = 1.0f - (float) queue[idx].age * 0.015f / ((float) MAX_AGE * 0.02f);
				Color c = new Color(f,f,f,f);
				// Use this information to draw.
				canvas.draw(texture, c, ox, oy, queue[idx].x, queue[idx].y, 0, scale, scale);
