...
public class Ship {
	...
	public float getDiameter(int n) {
		if (n==1){
			return SHIP_SIZE*DEFAULT_SCALE;
		}
	...
	public void move(float forward, float turn){
		...
		if (forward != 0.0f) {
			...
			vel.add(forward * (float)Math.cos(Math.toRadians (ang)) * THRUST_FACTOR,
					forward * (float)-Math.sin (Math.toRadians (ang)) * THRUST_FACTOR).scl(FORWARD_DAMPING);
		} else {  
	 ...
	 */
    public void drawShip(GameCanvas canvas, float n) {
    	if (shipSprite == null) {
    		return;
    	}
		...
		canvas.draw(shipSprite, stint, ox, oy, sx, sy, rotate, n, n);
		// Then draw the ship
		canvas.draw(shipSprite, tint, ox, oy, pos.x, pos.y, rotate, n, n);
	}
